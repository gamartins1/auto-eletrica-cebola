...
public interface PecaEstoqueRepository extends JpaRepository<PecaEstoque, Long> {
    ...
    @Override
    @Query(value = "SELECT * FROM tb_peca_estoque WHERE ativo = 1 AND quantidade_peca_estoque > 0", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM tb_peca_estoque WHERE nome_peca_estoque = ?1", nativeQuery = true)
